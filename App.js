...
const CSS = `
  ...
  *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
  ...
  .btn-outline {
    background:transparent; border:1px solid #5c4e22; color:#C9A84C;
  ...
  .btn-ghost {
    background:transparent; border:1px solid #332a17; color:#6b5e3e;
    font-family:'Space Mono',monospace; letter-spacing:.15em;
    text-transform:uppercase; cursor:pointer; transition: all .2s;
  }
  ...
  return (
    <>
      ...
      <div style={{ fontFamily:"'Cormorant Garamond',serif", background:G.black, minHeight:"100vh", color:G.text, position:"relative", zIndex:2 }}>
        {page === "landing" && <Landing go={setPage} scrolled={scrolled} />}
        {page === "library" && <Library go={setPage} search={search} setSearch={setSearch} filtered={filtered} selected={selected} setSelected={setSelected} activeVar={activeVar} setActiveVar={setActiveVar} userTier={userTier} onRedeemCode={() => setShowCodeModal(true)} />}
        {page === "pricing" && <Pricing go={setPage} onRedeemCode={() => setShowCodeModal(true)} />}
...
function Nav({ go, scrolled }) {
  return (
    <nav style={{
      ...
      <button onClick={() => go("landing")} style={{ background:"none", border:"none", cursor:"pointer", display:"flex", alignItems:"baseline", gap:6 }}>
...
// ── LIBRARY ───────────────────────────────────────────────────
function Library({ go, search, setSearch, filtered, selected, setSelected, activeVar, setActiveVar, userTier, onRedeemCode }) {
    ...
    <div style={{ background:G.black, minHeight:"100vh" }}>
      <div style={{ background:G.dark, borderBottom:`1px solid ${G.border}`, padding:"16px 40px", position:"sticky", top:0, zIndex:100, display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
        <div style={{ display:"flex", alignItems:"center", gap:24 }}>
          <button onClick={() => go("landing")} style={{ background:"none", border:"none", color:G.muted, cursor:"pointer", fontFamily:"'Space Mono',monospace", fontSize:10, letterSpacing:"0.2em" }}>← HOME</button>
      ...
      <div style={{ maxWidth:1100, margin:"0 auto", padding:"32px 40px" }}>
        {userTier === "free" && (
          <div style={{ background:`linear-gradient(135deg,#1a1200,#0d0d0d)`, border:`1px solid ${G.goldDim}`, padding:"20px 28px", marginBottom:32, display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:16 }}>
            ...
            <div style={{ display:"flex", gap:8 }}>
              <button onClick={() => href="https://buy.stripe.com/6oU4gBfB6g57dWA8xe5AQ00"} className="btn-outline" style={{ padding:"9px 20px", fontSize:10 }}>Vault — $15</button>
              <button onClick={() => href="https://buy.stripe.com/9B6dRb3So0694m0fZG5AQ01"} className="btn-gold" style={{ padding:"9px 20px", fontSize:10 }}>Codex — $50</button>
            </div>
          </div>
        )}
...
// ── PRICING ───────────────────────────────────────────────────
function Pricing({ go, onRedeemCode }) {
  return (
    <div style={{ background:G.black, minHeight:"100vh" }}>
